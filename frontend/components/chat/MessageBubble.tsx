...
const markdownComponents: React.ComponentProps<typeof ReactMarkdown>["components"] = {
  ...
  li: ({ children }) => (
    <li className="text-gray-800 leading-relaxed relative before:content-[''] pl-1">
      <span className="flex gap-2 items-start">
        <span className="mt-[6px] w-1.5 h-1.5 rounded-full bg-brand-600 shrink-0" />
        <span>{children}</span>
      </span>
    </li>
  ),
  ...
  code: ({ children, className }) => {
    ...
    if (isBlock) {
      return (
        <pre className="my-2 rounded-lg bg-gray-50 border border-border-default p-3 overflow-x-auto">
          <code className="text-xs font-mono text-gray-700">{children}</code>
        </pre>
      );
    }
    return (
      <code className="px-1 py-0.5 rounded text-[11px] bg-gray-100 text-brand-700 font-mono">
        {children}
      </code>
    );
  },
  ...
  // Blockquote
  blockquote: ({ children }) => (
    <blockquote className="border-l-2 border-brand-200 pl-3 my-2 text-gray-500 italic">
      {children}
    </blockquote>
  ),
  ...
  th: ({ children }) => (
    <th className="px-3 py-2 text-left text-[11px] font-semibold text-gray-700">{children}</th>
  ),
  td: ({ children }) => (
...
export default function MessageBubble({ message, contextQuery }: Props) {
  ...
  return (
    <div className={cn("flex gap-3 max-w-full", isUser ? "flex-row-reverse" : "flex-row")}>
      {/* Avatar */}
      {!isUser && (
        <div className="w-8 h-8 rounded-full bg-brand-600 flex items-center justify-center shrink-0 mt-1 shadow-xs">
      ...
      <div className={cn("flex flex-col gap-1 max-w-[78%]", isUser ? "items-end" : "items-start")}>
        ...
        <div
          className={cn(
            ...
            isUser
              ...
              : "bg-surface-card text-gray-800 border border-border-default shadow-xs rounded-tl-sm",
          )}
        >
          {isUser ? (
          ...
          <div className="flex flex-wrap items-center gap-1.5 mt-1 max-w-full">
            ...
            {message.citations.map((c) => (
              <Link
                ...
                className="inline-flex items-center gap-1 max-w-[220px] px-2 py-1 rounded-lg text-[10.5px] font-semibold text-brand-700 bg-brand-50 border border-brand-200 hover:bg-brand-100 transition-colors"
              >
                <svg aria-hidden="true" width="10" height="10" viewBox="0 0 12 12" fill="none" className="shrink-0">
        ...
        <div className="flex items-center gap-4 mt-1">
          {hasSteps && (
            ...
            >
              <svg aria-hidden="true" width="12" height="12" viewBox="0 0 12 12" fill="none" className={cn("transition-transform", reasoningOpen ? "rotate-180" : "")}>
          ...
          {!isUser && (
            <button
              ...
              className={cn(
                ...
                exporting
                  ? "text-brand-600 opacity-70 cursor-wait"
                  : "text-gray-400 hover:text-brand-600 cursor-pointer"
              )}
        ...
        {hasSteps && reasoningOpen && (
          <div className="w-full space-y-1.5 mt-1">
            {message.reasoning_steps!.map((step, i) => (
              <div
                key={i}
                className="flex items-start gap-2 px-3 py-2 rounded-lg bg-gray-50 border border-border-default text-xs"
              >
                <span className="w-5 h-5 rounded-full flex items-center justify-center shrink-0 text-[10px] font-bold bg-brand-50 text-brand-700">{i + 1}</span>
                <div>
                  <span className="font-semibold text-brand-700">{step.agent.replace("Agent","")}: </span>
