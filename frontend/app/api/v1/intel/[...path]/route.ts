/**
 * app/api/v1/intel/[...path]/route.ts
 *
 * Catch-all proxy: forwards every request under /api/v1/intel/*
 * to the FastAPI backend at API_BASE/api/v1/intel/*.
 *
 * Handles: GET, POST, PUT, PATCH, DELETE
 * Streaming: forwards ReadableStream responses (SSE /stream endpoint)
...
async function proxyRequest(
  ...
  // Build the upstream URL: /api/v1/intel/<rest-of-path>?<query>
  const subPath = params.path.join("/");
  const search = request.nextUrl.search ?? "";
  const upstreamUrl = `${API_BASE}/api/v1/intel/${subPath}${search}`;
  ...
  try {
    ...
    console.error("[intel proxy] upstream fetch failed:", err);
  ...
  // Detect SSE / streaming responses and pipe them through
    ...
    return new NextResponse(blob, {
      ...
      headers: {
        ...
        "Content-Disposition":
          ...
          "attachment; filename=iroko-brief.pdf",
      },
    });
  }
