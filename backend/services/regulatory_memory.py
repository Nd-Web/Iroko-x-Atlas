...
CBN/SEC change is detected, the system can surface context like:

    "A similar ruling happened in 2024. A large MFB paid a ₦2bn CBN fine.
     Lesson: file AML/CFT returns two weeks early to avoid penalties."
...
Usage::
    ...
    entry = await regulatory_memory.store_event(
        db,
        event_type="cbn_ruling",
        title="CBN ₦2bn Capital Adequacy Fine — Carbon MFB Q2 2026",
        description="CBN issued a ₦2bn fine to Carbon MFB for CAR falling to 8.7% ...",
        ncc_ref="CBN-CAR-001",
        outcome="Fine paid. Carbon MFB recapitalised within 90 days.",
        workspace_id="african-fintech-platform",
    )

    # Retrieve context for injection into an agent prompt
    context = await regulatory_memory.generate_historical_context(
        db,
        current_signal={"description": "CBN AML/CFT threshold breach detected — STR filing gap."},
    )
    # → "Similar past event: CBN ₦2bn Capital Adequacy Fine — Carbon MFB Q2 2026 (2026-04-01). ..."
"""
