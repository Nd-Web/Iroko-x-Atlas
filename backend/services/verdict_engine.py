...
Usage::
    ...
    output = verdict_engine.format_verdict_output(
        finding={"summary": "MTN QoS Q3 report is due in 5 days."},
        verdict=verdict,
        sources=[{"title": "NCC QoS Rules 2024", "url": "https://ncc.gov.ng/..."}],
        ncc_refs=["NCC-003"],
    )
...
class VerdictOutput(BaseModel):
    ...
    ncc_references : list[str]
        NCC regulation IDs referenced (e.g. ``["NCC-003", "NCC-001"]``).
...
class VerdictEngine:
    ...
    def generate_recommended_actions(
        ...
        if verdict == _GO:
            return [
                f"Proceed with the planned action{_ref_clause} — compliance gate passed.",
                ...
                (
                    ...
                    "with any new NCC enforcement notices."
                ),
            ]
        ...
        if verdict == _MONITOR:
            ...
            return [
                ...
                (
                    f"Cross-check against the latest NCC enforcement register{_ref_clause} "
                    f"before proceeding{_source_clause}."
                ),
                (
                    "Escalate to the Regulatory Affairs team if signal confidence does not "
                    "improve after the next Bright Data sweep. Do not proceed unilaterally."
                ),
            ]

        # NO-GO
        immediate_action = (
            f"Halt the planned action immediately — a compliance violation has been "
            f"detected{_ref_clause}."
        )
        notify_action = (
            "Notify the Chief Compliance Officer and NCC Regulatory Affairs team within "
            "24 hours as required by NCC-001 Section 73 (false/non-compliant submissions)."
        )
        remediation_action = (
