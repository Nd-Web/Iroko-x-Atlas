...
async def orchestrate_pipeline(
    query: str,
    org_id: str = "African Fintech Platform",
...
async def web_intel_pipeline(
    ...
    2. REGULATORY — compile_live_enforcement_rules() + CBN/SEC corpus diff
    3. GRAPH      — build signal correlation graph, find compound risks
    4. MEMORY     — inject institutional historical context
    5. COMPLIANCE — check query against live CBN/SEC rules
    ...
    yield _wi_step(
        "Regulatory", "cbn_scan",
        "Fetching live CBN enforcement updates and diffing against corpus…",
    )
    ...
    try:
        ...
        yield _wi_step(
            "Regulatory", "cbn_scan",
            (
                f"CBN scan complete. {len(base_regs)} base regulations in corpus. "
                f"Delta: {delta_str}."
            ),
            status="done",
            regulatory_summary={
    ...
    yield _wi_step(
        "Compliance", "cbn_check",
        f"Running query against live CBN rule corpus: \"{query[:100]}\"…",
    )
    ...
    try:
        ...
        finding = {
            "summary": (
                ...
                f"CBN violations: {violation_count}. "
        ...
        yield _wi_step(
            ...
            (
                ...
                f"{violation_count} CBN violation(s)."
            ),
            status="complete",
            verdict=final_verdict,
